...
class API(object):
    ...
    def _category(self, slug):
        return self._session.get(
            "/api/v2/android/play/page/categories/{}".format(slug)
        ).json()
    ...
    def show(self, slug):
        data = self._session.get(
            "/api/v2/android/play/page/shows/{}".format(slug)
        ).json()
    ...
    def search(self, query):
        ...
        return self._session.get("/api/v1/android/play/search", params=params).json()[
            "results"
        ]
    ...
    def get_brightcove_src(self, referenceID):
        ...
        resp = self._session.get(
            brightcove_url, headers={"BCOV-POLICY": BRIGHTCOVE_KEY}
        )
        data = resp.json()
