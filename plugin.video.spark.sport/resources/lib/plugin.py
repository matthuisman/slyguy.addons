...
def home(**kwargs):
    ...
    if not api.logged_in:
        ...
        folder.add_item(
            label=_(_.FREEMIUM, _bold=True),
            path=plugin.url_for(page, page_id="FREEMIUM"),
        )
...
def _process_rows(rows):
    ...
    for row in rows:
        ...
        if row.get("resourceType") == "epg/stations":
            ...
            if (
                row.get("customAttributes", {}).get("isLinearChannelInLiveEvent")
                != "true"
            ):
                item.context.append(
                    (
            ...
            item.path = plugin.url_for(
                ...
                play_type=settings.getEnum(
                    "live_play_type", PLAY_FROM_TYPES, PLAY_FROM_ASK
                ),
                _is_live=True,
            )
...
def playlist(output, **kwargs):
    ...
    with codecs.open(output, "w", encoding="utf8") as f:
        ...
        for channel in channels:
            f.write(
                u'#EXTINF:-1 tvg-id="{id}" tvg-name="{name}" tvg-logo="{logo}",{name}\n{path}\n'.format(
                    id=channel["id"],
                    name=channel["name"],
                    logo=IMG_URL.format(channel["pictureID"]),
