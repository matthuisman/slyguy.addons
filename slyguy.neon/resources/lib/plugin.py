...
def home(**kwargs):
    ...
    if not api.logged_in:
        folder.add_item(
            label=_(_.LOGIN, _bold=True), path=plugin.url_for(login), bookmark=False
        )
    else:
        folder.add_item(
            label=_(_.TV, _bold=True),
            path=plugin.url_for(menu, screen_id=TV_ID, title=_.TV),
        )
        folder.add_item(
            label=_(_.MOVIES, _bold=True),
            path=plugin.url_for(menu, screen_id=MOVIES_ID, title=_.MOVIES),
        )
        ...
        if not userdata.get("kid_lockdown", False):
            folder.add_item(
                ...
                art={"thumb": userdata.get("profile_icon")},
        ...
        folder.add_item(
            label=_.LOGOUT, path=plugin.url_for(logout), _kiosk=False, bookmark=False
        )
...
def _select_profile():
    ...
    for index, profile in enumerate(profiles):
        ...
        options.append(
            plugin.Item(label=profile["name"], art={"thumb": profile["avatar"]["uri"]})
        )
    ...
    index = gui.select(
        _.SELECT_PROFILE, options=options, preselect=default, useDetails=True
    )
    if index < 0:
        return
...
def _set_profile(profile, notify=True):
    ...
    if notify:
        gui.notification(
            ...
            icon=userdata.get("profile_icon"),
        )
